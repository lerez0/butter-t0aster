...
while IFS= read -r line; do
    ...
    if [[ $FS_TYPE == "btrfs" ]]; then
        if [[ -z "$DISK_ROOT" ]]; then
            DISK_ROOT="$PARTITION" # assign first BTRFS partition as /root
        else
            DISK_HOME="$PARTITION" # assign second BTRFS partition as /home
        fi
    fi
done < <(blkid -o export | grep -E "^DEVNAME|^TYPE" | paste - -)

if [[ -z "$DISK_ROOT" || -z "$DISK_HOME" ]]; then
    echo "ERROR could not detect both root and home BTRFS partitions > check disk layout"
    exit 1
fi
...
ROOT_MOUNT_POINT="/mnt" # create mount point for /root
HOME_MOUNT_POINT="/mnt/home" # create mount point for /home

mkdir -p $ROOT_MOUNT_POINT # ensure /root mount point exists
mkdir -p $HOME_MOUNT_POINT # ensure /home mount point exists

if [[ ! -d "$ROOT_MOUNT_POINT" || ! -d "$HOME_MOUNT_POINT" ]]; then
    echo "ERROR mount points do not exist or are not writable"
    exit 1
fi
